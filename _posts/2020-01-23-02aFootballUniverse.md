...
The **network-process perspective** of aFICSCHASS blog is centered in a **generative process** of *Levy flights* or *effective trajectories* that starts in an α point in any location inside the landscape or in a β source point in the *perimeter* or inside the landscape after a rule has been applied by the arbiter and; ends in an ω point with a contralateral atomic action that direct the ball to opponent's goal.
